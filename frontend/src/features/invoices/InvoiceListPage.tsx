    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Invoices will appear here once purchase orders pass Agentic RAG validation."
        />
      ) : (
                ...
                {data.data.map((inv) => (
                  <tr key={inv.id} className="hover:bg-slate-50/60 transition">
                    <td className="py-4 px-6 font-mono font-bold text-slate-900">
                      <Link to={`/invoices/${inv.id}`} className="hover:text-emerald-600">
                        {inv.invoiceNumber}
                      </Link>
                    </td>
                    <td className="py-4 px-6 font-mono text-xs text-slate-600">
                      <Link to={`/pos/${inv.poId}`} className="hover:underline">
                        {inv.poNumber}
                      </Link>
                    </td>
                    <td className="py-4 px-6 text-slate-800">{inv.customerName}</td>
                    <td className="py-4 px-6 font-mono text-xs text-slate-600">
                      {inv.gstNumber}
                    </td>
